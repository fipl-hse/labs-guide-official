...
class ExtractPairsTest(unittest.TestCase):
    ...
    def test_ideal_case(self):
        ...
        self.assertTrue(
            all(isinstance(pair, tuple) for pair in actual),
            "Elements of extract_pairs return value must be tuple",
        )

        actual_unordered = [set(pair) for pair in actual]
        self.assertTrue(
            all(pair in pairs for pair in actual_unordered), "There are incorrect pairs extracted"
        )
        self.assertTrue(
            all(pair in actual_unordered for pair in pairs), "Some of the pairs were not extracted"
        )
