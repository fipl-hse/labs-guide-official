...
from lab_2_classify_by_knn.main import calculate_distance_sparse, predict_language_knn_sparse
...
class PredictLanguageKnnSparseTest(unittest.TestCase):
    ...
    def test_predict_language_knn_sparse_ideal(self):
        ...
        second_text_vectors = [
            [[1, 0.2], [3, 0.1], [5, 0.49], [7, 0.3]],
            [[0, 0.1], [2, 0.4], [3, 0.1], [6, 0.34], [7, 0.3]],
            [[1, 0.2], [3, 0.1], [5, 0.49], [7, 0.3], [8, 0.35]],
            [[0, 0.11], [2, 0.34], [3, 0.1], [4, 0.12], [6, 0.8], [7, 0.12], [8, 0.1]],
            [[0, 0.1], [2, 0.4], [3, 0.1], [4, 0.1], [5, 0.11], [6, 0.34], [7, 0.3]],
            [[2, 0.4], [6, 0.6], [7, 0.3], [8, 0.35]],
        ]
        language_labels = ["en", "de", "en", "en", "de", "de"]
        expected = ["de", 0.43784]
        actual = predict_language_knn_sparse(
            first_text_vector, second_text_vectors, language_labels, 3
        )
    ...
    @pytest.mark.mark10
    @patch(
        "lab_2_classify_by_knn.main.calculate_distance_sparse",
    ...
    def test_calculate_distance_called(self, mock):
        ...
        predict_language_knn_sparse(first_text_vector, second_text_vectors, language_labels, 3)
    ...
    def test_predict_language_knn_sparse_bad_input(self):
        ...
        for bad_input in bad_inputs:
            actual = predict_language_knn_sparse(bad_input, bad_input, bad_input, bad_input)
            self.assertEqual(expected, actual)
