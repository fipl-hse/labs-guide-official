...
from lab_4_summarization_textrank.main import Sentence, SentenceEncoder, SentencePreprocessor
...
class SentenceEncoderTest(unittest.TestCase):
    ...
    preprocessor = SentencePreprocessor(
        ("sed", "ex", "ut", "est", "non", "in", "qui", "esse", "ea", "et", "do"),
        tuple(string.punctuation),
    )
    sentences = preprocessor.get_sentences(text)

    encoded_sentences = [
        (1011, 1014, 1010, 1015, 1006, 1008, 1007, 1002, 1003, 1001, 1012, 1009, 1004, 1005, 1013),
    ...
    def test_sentence_encoder_initialization_ideal(self):
        """
        Test for sentence encoder
        """
        encoder = SentenceEncoder()
        for attribute in ["_word2id", "_id2word"]:
            self.assertTrue(
                hasattr(encoder, attribute), f"Sentence encoder misses {attribute} attribute"
            )
    ...
    def test_encode_sentences_ideal(self):
        """
        Test for encode sentences ideal
        """
        encoder = SentenceEncoder()
    ...
    def test_encode_sentences_value_error(self):
        """
        Test for encoding sentences value error
        """
        encoder = SentenceEncoder()
        for wrong_input in [
            "text",
            ...
            4.0,
            ["word1", "word2"],
            [Sentence("word1", 0), Sentence("word2", 1)],
        ]:
