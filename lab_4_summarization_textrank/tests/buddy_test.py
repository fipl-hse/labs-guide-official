...
from lab_4_summarization_textrank.main import Buddy, Sentence, SentenceEncoder, SentencePreprocessor
...
class BuddyTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.TARGET_ANSWER_PATH = tests_path / "expected_answer.txt"

        cls.paths_to_texts = [str(path) for path in texts_path.glob("*.txt")]
        ...
        cls.buddy = Buddy(cls.paths_to_texts, stop_words, tuple(string.punctuation), idf)

    @pytest.mark.lab_4_summarization_textrank
    @pytest.mark.mark10
    def test_initialization_ideal(self):
        """
        Test for ideal initialization
        """
        attributes = [
            "_stop_words",
            "_punctuation",
    ...
    def test_knowledge_database(self):
        ...
        for text_properties in self.buddy._knowledge_database.values():
            self.assertTrue(
                all(key in text_properties for key in ["sentences", "keywords", "summary"])
            )

            self.assertTrue(text_properties["sentences"])
            self.assertIsInstance(text_properties["sentences"], tuple)
            self.assertTrue(
                all(isinstance(sentence, Sentence) for sentence in text_properties["sentences"])
            )
            ...
            self.assertTrue(
                all(isinstance(keyword, str) for keyword in text_properties["keywords"])
            )
    ...
    def test_find_texts_close_to_keywords(self):
        """
        Test for finding texts close to keywords
        """
        keywords = (
            "министр",
            "нефть",
    ...
    def test_find_texts_close_to_keywords_error(self):
        ...
        self.assertRaises(
            NoRelevantTextsError, self.buddy._find_texts_close_to_keywords, keywords, 10
        )
    ...
    def test_find_texts_close_to_keywords_invalid_input(self):
        """
        Test for finding texts close to keywords with invalid error
        """
        ...
        for invalid_input in invalid_inputs:
            self.assertRaises(
                ValueError, self.buddy._find_texts_close_to_keywords, keywords, invalid_input
            )
        invalid_inputs = [{5: 6}, 4.0, 6, -3]
        for invalid_input in invalid_inputs:
            self.assertRaises(
                ValueError, self.buddy._find_texts_close_to_keywords, invalid_input, n_texts
            )
