...
from lab_4_summarization_textrank.main import Sentence, SentencePreprocessor
...
class SentencePreprocessorTest(unittest.TestCase):
    ...
    def test_sentence_preprocessor_initialization_ideal(self):
        """
        Test ideal initialization of preprocessor
        """
        ...
        for attribute in ["_stop_words", "_punctuation"]:
            self.assertTrue(
                hasattr(preprocessor, attribute),
                f"Sentence preprocessor misses {attribute} attribute",
            )
    ...
    def test_sentence_preprocessor_initialization_value_error(self):
        """
        Test for error in initialization of preprocessor
        """
        ...
        for wrong_input in wrong_inputs:
            self.assertRaises(
                ValueError, sentence_preprocessor_initialization, wrong_input, ("1", "2", "3")
            )
            self.assertRaises(
                ValueError, sentence_preprocessor_initialization, ("1", "2", "3"), wrong_input
            )
    ...
    def test_split_by_sentence_value_error(self):
        """
        Test value error in sentence split
        """
        preprocessor = SentencePreprocessor((), tuple(string.punctuation))
    ...
    def test_get_sentences_value_error(self):
        """
        Test value error in getting sentences
        """
        stop_words = ("sed", "ex", "ut", "est", "non", "in", "qui", "esse", "ea", "et", "do")
    ...
    def test_preprocess_sentences_ideal(self):
        """
        Test for ideal sentences preprocessing
        """
        stop_words = ("sed", "ex", "ut", "est", "non", "in", "qui", "esse", "ea", "et", "do")
        preprocessor = SentencePreprocessor(stop_words, tuple(string.punctuation))
        sentences = tuple(
            Sentence(sentence, position) for position, sentence in enumerate(self.sentences)
        )
    ...
    def test_preprocess_sentences_value_error(self):
        """
        Test for value error in sentences preprocessing
        """
        stop_words = ("sed", "ex", "ut", "est", "non", "in", "qui", "esse", "ea", "et", "do")
        preprocessor = SentencePreprocessor(stop_words, tuple(string.punctuation))
